...
The dataset is vendored as a CSV (rows with missing values dropped, leaving 333 penguins), so no
...
the maximizer of `acc_test − λ·Ω(f)`. λ here is a selection meta-parameter, deliberately
distinct from the per-model fitting knob:
...
Ω is the number of non-zero coefficients because that is the natural analogue of
"number of leaves": both count the effective pieces of the model a human has to read. An L1 penalty drives coefficients to exactly zero, so increasing λ yields a
sparser, more interpretable linear model that uses fewer features.

On this dataset the tree's test accuracy already peaks at 3 leaves, so the slider picks
the 3-leaf tree for small λ and collapses to the 2-leaf tree only at large λ.
Regularization here prefers the simplest model that still achieves the best accuracy.

One caveat, also shown on the page: the brief's selection criterion uses `acc_test`,
so the test set participates in model selection and the displayed test accuracy is the
selection criterion itself. That makes it a slightly optimistic figure rather than an
...
* **Actionability caveat**: sampled counterfactuals need not be actionable or on the data
  manifold. A counterfactual that flips `year` ("had this penguin been measured in
...
* **ALE** accumulates the *local* effect, i.e. the integral of the partial derivative
  ...
  * **Decision tree: discretization.** A tree is piecewise-constant, so its derivative is
    zero almost everywhere with jumps at the splits, which makes the analytic route
    unusable. We instead bin
...
The difference is visible in the plots: the tree's PDP/ALE are step functions that jump at
